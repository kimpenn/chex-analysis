...
local({
    ...
    createBioGroupConfig <- function(bioGroups, bioGroup2species, bioGroup2cellType, bioGroup2harvest, bioGroup2drugGroup, bioGroup2hasTPA, bioGroup2TPADose, bioGroup2TPATime, bioGroup2hasMungBean, outFile) {
    ...
    createSampleInfoFull <- function(sampleInfoFile, sampleInfoFullFile, bioGroups = c("K562", "HumanAstroCulture", "HumanNeuronCulture", "MouseAstroCulture", "MouseNeuronCulture", "MouseNeuronSlice"), bioGroupConfigFile, sampleIDsOutFile = NULL, sampleIDsToExclude = NULL, exptIDsToExclude = NULL) {
    ...
    ChexTxCommDiff <- function(X, Y, Ycutoff = c("median", "zero")) {
        Ycutoff <- match.arg(Ycutoff)
        g <- intersect(rownames(X), rownames(Y))
        X <- X[g, ,drop = FALSE]; Y <- Y[g, ,drop = FALSE]
        gX <- rownames(X)[rowMeans(X) > 0]
        gY <- rownames(Y)[rowMeans(Y) > ifelse(Ycutoff == "median", median(rowMeans(Y)), 0)]
        Lintsect <- intersect(gX, gY)
        LXuniq <- setdiff(gX, gY)
        LYuniq <- setdiff(gY, gX)
        list(Comm = Lintsect, Xuniq = LXuniq, Yuniq = LYuniq)
    }

    enrichChexTxOverlap <- function(X, Y, Ycutoff = c("median", "zero"), pvalueCutoff = Inf, pAdjustMethod = "BH", qvalueCutoff = Inf, minGSSize = 5, maxGSSize = 500, TERM2GENE, TERM2NAME = NA) {
        Ycutoff <- match.arg(Ycutoff)
        g <- intersect(rownames(X), rownames(Y))
        X <- X[g, ,drop = FALSE]; Y <- Y[g, ,drop = FALSE]
        gX <- rownames(X)[rowMeans(X) > 0]
        gY <- rownames(Y)[rowMeans(Y) > ifelse(Ycutoff == "median", median(rowMeans(Y)), 0)]
        L <- list(gX, gY)
        Lintsect <- intersect(L[[1]], L[[2]])
        res <- clusterProfiler::enricher(gene = Lintsect, pvalueCutoff = pvalueCutoff, qvalueCutoff = qvalueCutoff, universe = g, pAdjustMethod = pAdjustMethod, minGSSize = minGSSize, maxGSSize = maxGSSize, TERM2GENE = TERM2GENE, TERM2NAME = TERM2NAME)
        res <- as.data.frame(res)
        rownames(res) <- NULL
        res
    }

    enrichChexUniq <- function(X, Y, Ycutoff = c("median", "zero"), pvalueCutoff = Inf, pAdjustMethod = "BH", qvalueCutoff = Inf, minGSSize = 5, maxGSSize = 500, TERM2GENE, TERM2NAME = NA) {
        Ycutoff <- match.arg(Ycutoff)
        g <- intersect(rownames(X), rownames(Y))
        X <- X[g, ,drop = FALSE]; Y <- Y[g, ,drop = FALSE]
        gX <- rownames(X)[rowMeans(X) > 0]
        gY <- rownames(Y)[rowMeans(Y) > ifelse(Ycutoff == "median", median(rowMeans(Y)), 0)]
        L <- list(gX, gY)
        Xuniq <- setdiff(L[[1]], L[[2]])
        res <- clusterProfiler::enricher(gene = Xuniq, pvalueCutoff = pvalueCutoff, qvalueCutoff = qvalueCutoff, universe = g, pAdjustMethod = pAdjustMethod, minGSSize = minGSSize, maxGSSize = maxGSSize, TERM2GENE = TERM2GENE, TERM2NAME = TERM2NAME)
        res <- as.data.frame(res)
        rownames(res) <- NULL
        res
    }

    enrichTxUniq <- function(X, Y, Ycutoff = c("median", "zero"), pvalueCutoff = Inf, pAdjustMethod = "BH", qvalueCutoff = Inf, minGSSize = 5, maxGSSize = 500, TERM2GENE, TERM2NAME = NA) {
        Ycutoff <- match.arg(Ycutoff)
        g <- intersect(rownames(X), rownames(Y))
        X <- X[g, ,drop = FALSE]; Y <- Y[g, ,drop = FALSE]
        gX <- rownames(X)[rowMeans(X) > 0]
        gY <- rownames(Y)[rowMeans(Y) > ifelse(Ycutoff == "median", median(rowMeans(Y)), 0)]
        L <- list(gX, gY)
        Yuniq <- setdiff(L[[2]], L[[1]])
        res <- clusterProfiler::enricher(gene = Yuniq, pvalueCutoff = pvalueCutoff, qvalueCutoff = qvalueCutoff, universe = g, pAdjustMethod = pAdjustMethod, minGSSize = minGSSize, maxGSSize = maxGSSize, TERM2GENE = TERM2GENE, TERM2NAME = TERM2NAME)
        res <- as.data.frame(res)
        rownames(res) <- NULL
        res
    }

    tabulateExprs <- function(exprs1, exprs2, cname1 = "Chex", cname2 = "Exprs", val1 = 0, cutoff2 = c("median", "0")) {
        g1 <- rownames(exprs1)
        g2 <- rownames(exprs2)
        g <- intersect(g1, g2)
        exprs <- cbind(exprs1[g, 1], exprs2[g, 1])
        colnames(exprs) <- c(cname1, cname2)
        cutoff2 <- match.arg(cutoff2)
        val2 <- switch(cutoff2, median = median(exprs2[, 1]), "0" = 0)
        binarizeTable(Table = exprs, cname1 = cname1, cname2 = cname2, val1 = val1, val2 = val2, cutoff2 = cutoff2)
        
    }

    vennChexTranscriptome <- function(X, Y, assayX = "CHEX", assayY = "RNA", featureX = "Gene", featureY = "Exon", Ycutoff = c("median", "zero"), filenameFormat = "Report/20181001/ChexTranscriptome/%s%s%sGt0%s%sGt%s.%s", sampleID, imagetype = "png", ...) {
        Ycutoff <- match.arg(Ycutoff)
        g <- intersect(rownames(X), rownames(Y))
        X <- X[g, , drop = FALSE]; Y <- Y[g, , drop = FALSE]
        gX <- rownames(X)[rowMeans(X) > 0]
        gY <- rownames(Y)[rowMeans(Y) > ifelse(Ycutoff == "median", median(rowMeans(Y)), 0)]
        labMap <- c("median" = "Median", "zero" = "0")
        labX <- sprintf("%s %s > %s", assayX, featureX, 0)
        labY <- sprintf("%s %s > %s", assayY, featureY, labMap[Ycutoff])
        L <- list(gX, gY)
        rotation <- ifelse(length(gX) < length(gY), 180, 0)
        names(L) <- c(assayX, assayY)
        if (!is.null(filenameFormat)) {
            filename <- sprintf(filenameFormat, assayX, featureX, sampleID, assayY, featureY, labMap[Ycutoff], imagetype)
        } else {
            filename <- NULL
        }
        VennDiagram::venn.diagram(L, filename = filename, imagetype = imagetype, total.population = length(g), rotation.degree = rotation, ...)
    }
