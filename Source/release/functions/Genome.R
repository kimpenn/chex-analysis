...
local({
    ...
    getCvgsPerFeaturePerChr <- function(cvgs, features, featureIDType = "ID", nbins = 500) {
        nfeatures <- length(features)
        if (nfeatures < 1) { 
            cvgsPerFeature <- matrix(0, ncol = nbins, nrow = 0)
        } else {
            featureIDs <- mcols(features)[[featureIDType]]
            strands <- strand(features)
            idx <- as.character(strands) == "-"
            ...
            if (!any(is.na(featureIDs)) && length(featureIDs) == nrow(cvgsPerFeature)) {
                rownames(cvgsPerFeature) <- featureIDs
            }
            if (any(idx)) {
                cvgsPerFeature[idx, ] <- matrix(rev(cvgsPerFeature[idx, ]), nrow = sum(idx))[sum(idx):1, ]
            }
        }
        cvgsPerFeature
    }
    ...
    getCvgsPerFeature <- function(cvgs, features, seqLevels = NULL, locType = c("start", "end", "middle"), extType = c("point", "range"), body = 5000, upstream = floor(body/2), downstream = floor(body/2), nbinsBody = 500, nbinsUpstream = floor(nbinsBody/2), nbinsDownstream = floor(nbinsBody/2), featureLengthLimit = c(-1, Inf), featureIDType = NULL, allFeatures = FALSE, ncores = 1) {
        locType <- match.arg(locType)
        extType <- match.arg(extType)

        nbinsExt <- list(Upstream = nbinsUpstream, Downstream = nbinsDownstream, Body = nbinsBody)
        ## Create unique IDs for each feature, and remove unnecessary metadata
        if (!is.null(featureIDType)) {
            featureIDs <- mcols(features)[[featureIDType]]
        } else if (!is.null(names(features))) {
            featureIDs <- names(features)
        } else {
            featureIDs <- as.character(features)
            if (!grepl("^chr", featureIDs[1])) {
                featureIDs <- make.names(features, unique = TRUE)
            }
        }
        allFeatureIDs <- featureIDs
        features <- GRanges(seqnames = seqnames(features), ranges = ranges(features), strand = strand(features), seqinfo = seqinfo(features), seqlengths = seqlengths(features), ID = featureIDs)

        ## Filter chromosomes that do not exist in features 
        cvgSeqLevels <- names(cvgs)
        featureSeqLevels <- seqlevels(features)
        commonSeqLevels <- intersect(cvgSeqLevels, featureSeqLevels)
        if (length(commonSeqLevels) == 0) stop("No chromosomes in common!")
        if (!is.null(seqLevels)) commonSeqLevels <- intersect(commonSeqLevels, seqLevels)
        if (length(commonSeqLevels) == 0) stop("Your specified chromosomes are all outside the scope!")
        nSeqLevels <- length(commonSeqLevels)
        missingSeqLevels <- setdiff(cvgSeqLevels, commonSeqLevels)
        if (length(missingSeqLevels) > 0) {
            message("Chromosomes dropped from cvgs are:\n", paste(missingSeqLevels, collapse = "\t"))
        }
        cvgs <- cvgs[commonSeqLevels]
        idx <- as.character(seqnames(features)) %in% commonSeqLevels
        features <- features[idx]

        ## Filter features whose body is too long or too short when extType == `range`
        featureLengths <- width(features)
        if (extType == "point") {
            regions <- c("Upstream", "Downstream")
            idx <- featureLengths >= featureLengthLimit[1] & featureLengths <= featureLengthLimit[2]
        } else if (extType == "range") {
            regions <- c("Upstream", "Body", "Downstream")
            if (!is.numeric(nbinsBody)) stop("When extType is range, nbinsBody must be set")
            idx <- featureLengths >= max(featureLengthLimit[1], nbinsBody) & featureLengths <= featureLengthLimit[2]
        }
        features <- features[idx]

        ## Extend feature to its upstream and downstream
        ## Make feature a point location if extType is point
        if (extType == "point") {
            featureStrands <- strand(features)
            featureSeqnames <- seqnames(features)
            featureStarts <- ifelse(featureStrands == "-", end(features), start(features))
            featureEnds <- ifelse(featureStrands == "-", start(features), end(features))
            featureMids <- round(0.5 * (start(features) + end(features)))
            featureLoc <- switch(locType, start = featureStarts, end = featureEnds, middle = featureMids)
            start(features) <- end(features) <- featureLoc
            featuresUpstream <- getUpstream(features, upstream)
            featuresDownstream <- getDownstream(features, downstream)
            featuresExt <- list(Upstream = featuresUpstream, Downstream = featuresDownstream)
        } else if (extType == "range") {
            featuresUpstream <- getUpstream(features, upstream)
            featuresDownstream <- getDownstream(features, downstream)
            featuresExt <- list(Upstream = featuresUpstream, Body = features, Downstream = featuresDownstream)
        }
        
        ## Filter features whose upstream or downstream is shorter than nbinsUpstream or nbinsDownstream 
        idx <- rep(TRUE, length(features))
        for (region in c("Upstream", "Downstream")) {
            i <- width(featuresExt[[region]]) >= nbinsExt[[region]]
            if (any(!i)) warning(sum(!idx), " features in ", region, " region is going to be filtered due to being shorter than nbins")
            idx <- idx & i
        }
        features <- features[idx]
        featuresExt <- lapply(featuresExt, function(features) features[idx])
        featureIdxBySeqLevel <- split(seq(features), f = seqnames(features))[commonSeqLevels]
        featuresExtBySeqLevel <- lapply(featuresExt, function(features) lapply(featureIdxBySeqLevel, function(idx) features[idx]))
        featuresExtBySeqLevel <- lapply(featuresExtBySeqLevel, as, Class = "GRangesList") # Class = "RangesList" does not work

        ## Compute bin-averaged coverage for each region 
        cvgsBySeqLevel <- NULL
        for (region in regions) {
            if (ncores == 1) {
                cvgsBySeqLevel[[region]] <- lapply(commonSeqLevels, function(seqLevel) { 
                    message(region, "\t", seqLevel)
                    getCvgsPerFeaturePerChr(cvgs = cvgs[[seqLevel]], features = featuresExtBySeqLevel[[region]][[seqLevel]], nbins = nbinsExt[[region]])})
            } else {
                cvgsBySeqLevel[[region]] <- mclapply(commonSeqLevels, function(seqLevel) { 
                    message(region, "\t", seqLevel)
                    getCvgsPerFeaturePerChr(cvgs = cvgs[[seqLevel]], features = featuresExtBySeqLevel[[region]][[seqLevel]], nbins = nbinsExt[[region]])}, mc.cores = ncores)
            }
            names(cvgsBySeqLevel[[region]]) <- commonSeqLevels
        }

        ## Assemble average coverage matrices from upstream, body (if "range"), and downstream
        if (extType == "point") {
            cvgsByFeature <- sapply(commonSeqLevels, function(seqLevel) {
                cbind(cvgsBySeqLevel[["Upstream"]][[seqLevel]], cvgsBySeqLevel[["Downstream"]][[seqLevel]])
            }, simplify = FALSE) 
        } else if (extType == "range") {
            cvgsByFeature <- sapply(commonSeqLevels, function(seqLevel) {
                cbind(cvgsBySeqLevel[["Upstream"]][[seqLevel]], cvgsBySeqLevel[["Body"]][[seqLevel]], cvgsBySeqLevel[["Downstream"]][[seqLevel]])
            }, simplify = FALSE)
        }
        cvgsByFeature <- do.call(rbind, cvgsByFeature)
        if (allFeatures) {
            nbins <- ncol(cvgsByFeature)
            featureIDs <- rownames(cvgsByFeature)
            missingFeatureIDs <- setdiff(allFeatureIDs, featureIDs)
            nMissing <- length(missingFeatureIDs)
            if (nMissing > 0) {
                fill <- matrix(NA, nrow = nMissing, ncol = nbins)
                rownames(fill) <- missingFeatureIDs
                cvgsByFeature <- rbind(cvgsByFeature, fill)
                cvgsByFeature <- cvgsByFeature[as.character(allFeatureIDs), ]
            }
        }
        Matrix::Matrix(cvgsByFeature)
    }
